...
export default function Layout({ children }) {
    ...
    <div className="space-y-0.5">
      ...
      {items.map((item) => {
        ...
        return (
          ...
          >
            <Icon className={`w-4 h-4 ${active ? 'text-white' : 'text-[#555568] group-hover:text-[#8888a0]'}`} />
  ...
  const SidebarContent = ({ isMobile = false }) => (
    <aside className={`fixed top-0 h-full ${isMobile ? 'left-0 w-60 z-50' : 'left-0 w-60 hidden md:flex'} bg-[#111118] border-r border-[#1e1e2e] flex-col justify-between transition-all duration-200`}>
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 py-4 border-b border-[#1e1e2e]">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-[13px] font-semibold text-white tracking-tight">PropostaFácil</h2>
              <p className="text-[10px] text-[#555568] font-medium">Plataforma Comercial</p>
            </div>
          </div>
        </div>

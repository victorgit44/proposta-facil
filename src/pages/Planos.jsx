...
export default function Planos() {
  ...
  return (
    <div className="p-6 md:p-12 max-w-7xl mx-auto space-y-16 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        {planosDisponiveis.map((plano, index) => {
          ...
          return (
            <div
              key={index}
              className={`relative rounded-lg p-6 bg-[#111118] border flex flex-col justify-between transition-all duration-300 ${
                isCurrentPlan
                  ? 'border-emerald-500/80 ring-1 ring-emerald-500/20'
                  : plano.popular
                  ? 'border-blue-600/80 shadow-lg shadow-blue-600/10'
                  : 'border-[#1e1e2e] hover:border-[#2a2a3e]'
              }`}
            >
              {/* Badges */}
              {plano.popular && !isCurrentPlan && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded bg-blue-600 text-white text-[10px] font-medium uppercase tracking-wider shadow-md">
                  Mais Escolhido
                </div>
              )}

              {isCurrentPlan && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded bg-emerald-600 text-white text-[10px] font-medium uppercase tracking-wider shadow-md">
                  Seu Plano Ativo
                </div>
              )}
              ...
              <div>
                <div className="flex items-center justify-between mb-5">
                  <div className="w-10 h-10 rounded-lg bg-blue-600/10 text-blue-400 flex items-center justify-center font-medium">
                    <Icon className="w-5 h-5" />
                  </div>
                  <span className="px-2 py-0.5 rounded text-xs font-medium bg-[#1a1a24] text-[#8888a0]">
                    {plano.nome}
                  </span>
                </div>

                {/* Price */}
                <div className="mb-5 pb-5 border-b border-[#1e1e2e]">
                  <div className="flex items-baseline gap-1">
                    <span className="text-3xl font-semibold text-white tracking-tight tabular-nums">{preco}</span>
                    <span className="text-[#555568] text-xs font-medium">/mês</span>
                  </div>
                  <p className="text-[11px] text-[#555568] mt-1 font-medium">{plano.subtexto}</p>
                </div>
              ...
              <button
                ...
                className={`w-full py-2.5 px-4 rounded-lg font-medium text-xs transition duration-200 flex items-center justify-center gap-2 cursor-pointer ${
                  isCurrentPlan
                    ? 'bg-[#1a1a24] text-[#555568] cursor-not-allowed border border-[#1e1e2e]'
                    : plano.popular
                    ? 'bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-600/20'
                    : 'bg-[#1a1a24] hover:bg-white/10 text-[#8888a0] hover:text-white border border-[#1e1e2e]'
                }`}
              >
      ...
      {/* Trust Badges */}
      <div className="p-4 rounded-lg bg-[#111118] border border-[#1e1e2e] grid grid-cols-1 md:grid-cols-3 gap-4 text-center text-xs text-[#8888a0]">
      ...
      <div className="max-w-3xl mx-auto space-y-6 pt-4">
        <div className="text-center">
          <h3 className="text-xl font-semibold text-white tracking-tight">Perguntas Frequentes</h3>
          <p className="text-xs text-[#8888a0] mt-1">Dúvidas comuns sobre planos e pagamento</p>
        </div>

        <div className="space-y-3">
          {faqs.map((faq, idx) => (
            <div key={idx} className="p-4 rounded-lg bg-[#111118] border border-[#1e1e2e] space-y-1.5">
              <h4 className="text-sm font-medium text-white flex items-center gap-2">
                <HelpCircle className="w-4 h-4 text-blue-400 shrink-0" />
                <span>{faq.q}</span>
              </h4>
              <p className="text-xs text-[#8888a0] leading-relaxed pl-6">{faq.a}</p>
            </div>
          ))}
        </div>
      </div>

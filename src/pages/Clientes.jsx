    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-[#1e1e2e]">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl font-semibold text-white tracking-tight">Diretório de Clientes & Empresas</h1>
            <span className="px-2 py-0.5 text-[10px] font-medium uppercase bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded">
              Base CRM
            </span>
          </div>
          <p className="text-sm text-[#8888a0] mt-1">Gestão centralizada dos contatos comerciais e histórico de faturamento por cliente.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {clientesList.length === 0 ? (
          <div className="col-span-full p-12 text-center text-[#555568] rounded-lg bg-[#111118] border border-[#1e1e2e]">
            Nenhum cliente cadastrado até o momento.
          </div>
        ) : (
          clientesList.map((cliente, idx) => (
            <div key={idx} className="p-5 rounded-lg bg-[#111118] border border-[#1e1e2e] hover:border-[#2a2a3e] transition space-y-4 group">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-semibold text-white tracking-tight group-hover:text-blue-400 transition">{cliente.nome}</h3>
                    <p className="text-xs text-[#8888a0]">{cliente.empresa}</p>
                  </div>
                </div>
                <span className="px-2 py-0.5 text-[10px] font-medium bg-[#1a1a24] text-[#8888a0] rounded">
                  {cliente.propostasCount} propostas
                </span>
              </div>

              <div className="space-y-2 text-xs text-[#8888a0]">
                <div className="flex items-center gap-2">
                  <Mail className="w-3.5 h-3.5 text-[#555568]" />
                  <span>{cliente.email}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Phone className="w-3.5 h-3.5 text-[#555568]" />
              ...
              <div className="pt-3 border-t border-[#1e1e2e] flex items-center justify-between">
                <div>
                  <span className="text-[10px] font-medium uppercase tracking-wider text-[#555568] block">Total Aprovado</span>
                  <span className="text-base font-semibold text-emerald-400 tabular-nums">{formatCurrency(cliente.totalComprado)}</span>
                </div>
                <button
                  onClick={() => navigate('/propostas/criar')}
                  className="p-1.5 text-[#555568] hover:text-white hover:bg-[#1a1a24] rounded transition cursor-pointer"
                >

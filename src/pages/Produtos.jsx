    ...
    <div className="p-6 md:p-10 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-6 border-b border-slate-800/80">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Catálogo de Produtos & Serviços</h1>
          ...
          <p className="text-xs text-slate-400 mt-1">Cadastre seus produtos e preços padronizados para inserção rápida em propostas.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          ...
          filtered.map((item) => (
            <div key={item.id} className="p-6 rounded-3xl bg-slate-900/90 border border-slate-800/90 hover:border-slate-700 transition flex flex-col justify-between space-y-4 shadow-lg group">
              <div className="space-y-3">
                ...
                <h3 className="text-base font-bold text-white group-hover:text-blue-400 transition">{item.nome}</h3>
                <p className="text-xs text-slate-400 leading-relaxed">{item.descricao || 'Sem descrição cadastrada.'}</p>
              </div>

              <div className="pt-4 border-t border-slate-800/80 flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase text-slate-500">Preço Unitário</span>
                <span className="text-base font-black text-emerald-400">{formatCurrency(item.preco_unitario)}</span>
              </div>
            </div>
          ))
        )}
